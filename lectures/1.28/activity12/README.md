...
The `bagbugs.tar` file contains buggy versions of bag and bagtest. Unpack the tar file using

```bash
tar -xvf bagbugs.tar
```

Then `cd bagbugs` and run `make`. The `bagbugstest` is a testing program for the `bagbugs` functions. 
